...
class NETATest : public ::testing::Test
{
    ...
    void testNETA(const Species &sp, const NETADefinition &neta, const std::vector<int> &matchingIndices)
    {
...
TEST_F(NETATest, Matching)
{
    ...
    // Atom in six-membered ring - basic_ atoms 0-5
    EXPECT_TRUE(neta.create("ring(size=6)"));
    testNETA(basic_, neta, {0, 1, 2, 3, 4, 5});

    // Hydrogen atom attached to atom within a six-membered ring - basic_ atoms 6-9
    EXPECT_TRUE(neta.create("nbonds=1,-C(ring(size=6))"));
    testNETA(basic_, neta, {6, 7, 8, 9});

    // Atom at junction of four and six-membered ring - basic_ atoms 0-1
    EXPECT_TRUE(neta.create("ring(size=6), ring(size=4)"));
    testNETA(basic_, neta, {0, 1});

    // Atom at junction of four and six-membered ring, adjacent to nitrogen - basic_ atoms 1
    EXPECT_TRUE(neta.create("ring(size=6), ring(size=4), -N"));
    testNETA(basic_, neta, {1});

    // Atom adjacent to nitrogen, but not in a ring - basic_ atoms 12
    EXPECT_TRUE(neta.create("-N,ring(n=0)"));
    testNETA(basic_, neta, {12});

    // Hydrogen atoms present in CH3 group - basic_ atoms 13,14,15
    EXPECT_TRUE(neta.create("-C(nh=3),nbonds=1"));
    testNETA(basic_, neta, {13, 14, 15});
}
