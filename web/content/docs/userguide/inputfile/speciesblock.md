...
|`CoordinateSets`|[`CoordinatesFileAndFormat`]({{< ref "coordinates" >}})|--|Sets a file from which coordinates for the species are read in, and which are used when adding multiple copies of the species to a [configuration]({{< ref "configurationblock" >}}) in its generator definition.|
