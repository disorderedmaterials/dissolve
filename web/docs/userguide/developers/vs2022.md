...
A specific version of Qt can be specified for installation via the `-qtVersion` parameter (i.e. 6.7.2).
...
2) Conan relies on importing its own `conan_provider` from the web at configure time. Occasionally, configuring the cache leads to an unexplained CMake error `conan install failed=1`, probably due to failure to download this. If this happens try deleting the cache and reconfiguring, or ultimately, manually delete the build folder, restart Visual Studio and reconfigure from scratch.

### Build project

Once configuration is complete, make sure to select the correct target executable from the `startup item` dropdown in Visual Studio, or any available target in the `CMake Targets View`.
The main Dissolve executable will be one of:
- `Dissolve.exe` (CLI version)
- `Dissolve-GUI.exe` (GUI version)
- `Dissolve-GUI-QML.exe` (GUI version 2)
