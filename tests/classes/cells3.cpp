...
class CellsEnergyTest : public ::testing::Test
{
    ...
    CellsEnergyTest() : dissolve_(coreData_)
    {
        ...
        arType->setName("Ar");
        arType->interactionPotential().setFormAndParameters(ShortRangeFunctions::Form::LennardJones, "epsilon=0.77404 sigma=3.445996");
    ...
    double tabulatedEnergyNoCells(Configuration *cfg, double cutoff)
    {
        ...
        auto energy = 0.0;
        dissolve::for_each_pair(ParallelPolicies::seq, cfg->molecules().begin(), cfg->molecules().end(), [&](int i, const auto &molI, int j, const auto &molJ) {
    ...
    void testEnergy(Configuration *cfg, const std::tuple<double, double, double, double> &state)
    {
        ...
        EXPECT_NEAR(tabulatedEnergyNoCells(cfg, rCut), kernel->totalPairPotentialEnergy(false, ProcessPool::PoolStrategy).total(), 1.0e-6);
...
TEST_F(CellsEnergyTest, Cubic)
{
    ...
    CoordinateImportFileFormat importer("dlpoly/argon/cubic/big_argon.CONFIG", CoordinateImportFileFormat::CoordinateImportFormat::DLPOLY);
    ...
    std::vector<std::tuple<double, double, double, double>> states = {
            {9.0, 5.0, -8.077004e3, -0.678949e3}, {9.0, 7.0, -8.077004e3, -0.678949e3}, {9.0, 9.0, -8.077004e3, -0.678949e3},  {9.0, 12.0, -8.077004e3, -0.678949e3}};
    //        {12.0, 5.0, -9.748103e3, -0.335651e3}, {12.0, 7.0, -9.748103e3, -0.335651e3}, {12.0, 9.0, -9.748103e3, -0.335651e3}, {12.0, 12.0, -9.748103e3, -0.335651e3},
    //        {15.0, 5.0, -9.747836e3, -0.171877e3}, {15.0, 7.0, -9.747836e3, -0.171877e3}, {15.0, 9.0, -9.747836e3, -0.171877e3}, {15.0, 15.0, -9.747836e3, -0.171877e3}};
...
TEST_F(CellsEnergyTest, Monoclinic)
{
    ...
    // Load the test coordinates
    CoordinateImportFileFormat importer("dlpoly/argon/monoclinic/big_argon.CONFIG", CoordinateImportFileFormat::CoordinateImportFormat::DLPOLY);
    ASSERT_TRUE(importer.importData(cfg));

    // Test range of cell sizes for various cutoffs - should make no difference to total energy
    std::vector<std::tuple<double, double, double, double>> states = { {9.0, 5.0, -9.747556e3, -0.794931e3} };
//        {9.0, 5.0, -9.747556e3, -0.794931e3}, {9.0, 7.0, -9.747556e3, -0.794931e3}, {9.0, 9.0, -9.747556e3, -0.794931e3},  {9.0, 12.0, -9.747556e3, -0.794931e3},
//        {12.0, 5.0, -9.748103e3, -0.335651e3}, {12.0, 7.0, -9.748103e3, -0.335651e3}, {12.0, 9.0, -9.748103e3, -0.335651e3}, {12.0, 12.0, -9.748103e3, -0.335651e3},
//        {15.0, 5.0, -9.747836e3, -0.171877e3}, {15.0, 7.0, -9.747836e3, -0.171877e3}, {15.0, 9.0, -9.747836e3, -0.171877e3}, {15.0, 15.0, -9.747836e3, -0.171877e3}};
