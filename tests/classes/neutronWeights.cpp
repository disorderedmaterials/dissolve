...
TEST(NeutronWeightsTest, NullWater)
{
    ...
    nwts = NeutronWeights(speciesPopulations, isotopologues, Exchangeables({molecules.atHW()->name()}));
    EXPECT_NEAR(pow(Sears91::boundCoherent(Sears91::O_Natural) / 3.0, 2) / 100.0, nwts.boundCoherentSquareOfAverage(), 1.0e-6);
}
...
TEST(NeutronWeightsTest, Mix)
{
    ...
    EXPECT_NEAR(1.0 / 5.0, nwts.isotopeMix().fraction(molecules.atOW().get()), 1.0e-6);
    EXPECT_NEAR(2.0 / 5.0, nwts.isotopeMix().fraction(molecules.atHW().get()), 1.0e-6);
    EXPECT_NEAR(2.0 / 5.0, nwts.isotopeMix().fraction(molecules.atN().get()), 1.0e-6);
