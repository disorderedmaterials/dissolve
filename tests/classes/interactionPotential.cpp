...
TEST(InteractionPotentialTest, ParameterNames)
{
    ...
    // Valid parameters
    EXPECT_TRUE(srPotential.parseParameters(std::format("epsilon={} sigma={}", epsilon, sigma)));
    ...
    EXPECT_TRUE(srPotential.parseParameters(std::format("sigma={} epsilon={}", sigma, epsilon)));
    EXPECT_EQ(srPotential.nParameters(), 2);
    EXPECT_DOUBLE_EQ(srPotential.parameters()[0], epsilon);
    EXPECT_DOUBLE_EQ(srPotential.parameters()[1], sigma);

    // Wrong number of parameters
    EXPECT_FALSE(srPotential.parseParameters(std::format("sigma={} epsilon={} suspect={}", sigma, epsilon, 1.0)));

    // Invalid parameter
    EXPECT_FALSE(srPotential.parseParameters(std::format("sigma={} suspect={}", sigma, epsilon)));
}
...
TEST(InteractionPotentialTest, DynamicParameterNames1)
{
    ...
    EXPECT_TRUE(torsionPotential.parseParameters(
        joinStrings(std::vector<int>{1, 2, 3, 4, 5}, " ", [](const auto n) { return std::format("k{}={}", n, n); })));
    ...
    EXPECT_TRUE(torsionPotential.parseParameters(
        joinStrings(std::vector<int>{5, 2, 1, 3, 4}, " ", [](const auto n) { return std::format("k{}={}", n, n); })));
    EXPECT_EQ(torsionPotential.nParameters(), 5);
    EXPECT_EQ(torsionPotential.parametersAsString(), "k1=1 k2=2 k3=3 k4=4 k5=5");
    for (auto n = 0; n < 5; ++n)
        EXPECT_DOUBLE_EQ(torsionPotential.parameters()[n], n + 1);
