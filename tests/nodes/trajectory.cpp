...
TEST(TrajectoryNodesTest, RoundTrip)
{
    ...
    ASSERT_TRUE(trajectoryImport->setOption("FilePath", std::filesystem::path(importFile.path())));
    ...
    ASSERT_TRUE(trajectoryExport->setOption("FilePath", std::filesystem::path(exportFile.path())));
