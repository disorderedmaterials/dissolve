...
{
    // Test data
    Data3D referenceSDF_;

    protected:
    // Prepare any necessary test data
    void prepareTestData() override
    {
        ASSERT_TRUE(ImportDLPUtilsPDensNode::read(referenceSDF_, "dlpoly/water267-analysis/water-267-298K.11.pdens"));
    }
    ...
    void performTests() override
    {
        ...
        ASSERT_TRUE(sdf);
        EXPECT_TRUE(testData3D(sdf->sdf(), "SDF", referenceSDF_, "dlpoly/water267-analysis/water-267-298K.11.pdens", 0.13));
    }
};

TEST_F(SDFNodeWaterTest, Water) { go(); }

class SDFNodeBenzeneTest : public TestGraphFixture
{
    // Test data
    Data3D referenceSDF_;
    ...
    void prepareTestData() override
    {
        ASSERT_TRUE(ImportDLPUtilsPDensNode::read(referenceSDF_, "dlpoly/benzene181/benzene181.11.pdens"));
    }
    // Perform graph construction
    void constructGraph() override
    {
        testGraph_.createConfiguration("Box", {{"species/benzene.toml", 181}},
                                       {29.925089931000, 29.925089931000, 29.925089931000});

        // Create trajectory iterator
        auto iterator = testGraph_.appendTrajectoryIterator("ImportXYZTrajectory", "dlpoly/benzene181/benzene181.xyz");
        ASSERT_TRUE(iterator);
        ...
        auto sdf = dynamic_cast<SDFNode *>(iterator->createNode("SDF"));
        ASSERT_TRUE(sdf);
        auto *benzene = testGraph_.findNode("Benzene")->getOutputValue<const Species *>("Species");
        ASSERT_TRUE(benzene);
    ...
    void performTests() override
    {
        ...
        auto *sdf = dynamic_cast<SDFNode *>(iterator->findNode("SDF"));
        ASSERT_TRUE(sdf);
        EXPECT_TRUE(testData3D(sdf->sdf(), "SDF", referenceSDF_, "dlpoly/benzene181/benzene181.11.pdens", 0.3));
    }
};

TEST_F(SDFNodeBenzeneTest, Water) { go(); }
