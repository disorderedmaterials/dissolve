...
TEST(GraphArgonTest, InitSimulation)
{
    ...
    TestGraph testGraph;
    testGraph.createConfiguration("Box", {{[] { return createAtomicSpecies(Elements::Ar); }, 1000}}, 0.0213);
...
TEST(GraphArgonTest, AllCorrelations)
{
    // Set up the test graph
    TestGraph testGraph;
    EXPECT_TRUE(testGraph.createConfiguration("Box", {{[] { return createAtomicSpecies(Elements::Ar); }, 1000}}, 0.0213));
