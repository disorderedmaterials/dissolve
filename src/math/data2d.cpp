...
const Array<double>& Data2D::constXAxis() const
{
	return x_;
}
...
const Array<double>& Data2D::constYAxis() const
{
	return y_;
}
...
const Array< double >& Data2D::values2DLinear() const
{
	return values_.linearArray1D();
}

double Data2D::constValue(int index)
{
	Array<double> array = values2DLinear();
	return array[index];
}

// Return number of values present in whole dataset
int Data2D::nValues() const
{
...
{
	if (hasError_) Messenger::warn("Adding an error array to a Data2D that already has one...\n");
...
bool Data2D::valuesHaveErrors() const
{
	return hasError_;
}
...
{
	if (!hasError_) Messenger::warn("This Data2D (name='%s', tag='%s') has no errors to return, but constErrors2D() was requested.\n", name(), objectTag());
...
// Return class name
const char* Data2D::itemClassName()
{
	return "Data2D";
}
...
{
#ifdef PARALLEL
	if (!procPool.equality(x_)) return Messenger::error("Data2D x axis values not equivalent.\n");
	if (!procPool.equality(y_)) return Messenger::error("Data2D y axis values not equivalent.\n");
	if (!procPool.equality(values_)) return Messenger::error("Data2D values not equivalent.\n");
	if (!procPool.equality(hasError_)) return Messenger::error("Data2D error flag not equivalent.\n");
	if (!procPool.equality(errors_)) return Messenger::error("Data2D error values not equivalent.\n");
#endif
	return true;
}
