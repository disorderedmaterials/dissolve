...
bool DUQMath::gjInvert(Array2D<double>& A)
{
	...
	for (n=0; n<rank; ++n)
	{
		...
		for (m=0; m<rank; ++m) array[pivotcol*rank+m] *= element;
		...
		for (row=0; row<rank; ++row)
		{
			...
			for (m=0; m<rank; ++m) array[row*rank+m] = array[row*rank+m] - array[pivotcol*rank+m] * element;
		}
	}
