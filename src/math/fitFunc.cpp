...
namespace DissolveFit
{
    ...
    */
     ...
     * Peaks
    */
    Peaks::Peaks(const std::vector<double> &values, const std::vector<double> &domain) 
    ...
    void Peaks::setThreshold(double threshold)
    {
        threshold_ = threshold;
    }

    // Set horizontal isolation radius for peaks
    void Peaks::setIsolation(double radius)
    {
        isolation_ = radius;
    }

    // Reset the threshold and isolation constraints
    void Peaks::resetConstraints()
    {
        threshold_=-10e9, isolation_=0;
    }

    // Check if neighbouring points correspond to a local maximum
    bool Peaks::isLocalMaximum(double *points)
    {
    ...
    {
        return (
            ((points[1]-points[0] > 0) || ((points[1]-points[0] < 0)) ) && (points[2]-points[1] == 0)
        ) ||
        (
    ...
    std::vector<Peaks::Peak1D> Peaks::top(int n, std::vector<Peaks::Peak1D> peaks)
    {
        ...
        {
            bool withinRadius = std::any_of(
                isolatedPeaks.begin(), isolatedPeaks.end(), [this, &peak](const auto &p)
                {
                    //return compare(p, peak);
                    return (p.index != peak.index) && ((p.valueAt > peak.valueAt - isolation_) && (p.valueAt < peak.valueAt + isolation_));
                }
            );
    ...
    std::vector<Peaks::Prominence1D> Peaks::prominences(const std::vector<Peaks::Peak1D> &peaks, bool heightOrder)
    {
        ...
        for (const auto &peak : peaks)
        {
            ...
            while (heightRef == 0)
            {
                ...
                {
                    Messenger::print("Could not calculate reference height for prominence of peak at index {} (no local minima or inflection point found in either direction)\n", peak.index);
                    break;
                }
                else
                    iLeft--, iRight++;
