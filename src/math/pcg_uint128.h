...
inline bitcount_t trailingzeros(uint8_t v) { return trailingzeros(uint32_t(v)); }

inline bitcount_t trailingzeros(uint16_t v) { return trailingzeros(uint32_t(v)); }
...
template <typename UInt> inline bitcount_t clog2(UInt v) { return flog2(v) + ((v & (-v)) != v); }
