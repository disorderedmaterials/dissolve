...
const ProcessPool &TaskContext::processPool() const { return processPool_; }

Dissolve &TaskContext::dissolve()
{
    if (!dissolve_)
        throw(std::runtime_error("Dissolve is not set in this task's context.\n"));
    return dissolve_->get();
}

const Dissolve &TaskContext::dissolve() const
{
    if (!dissolve_)
        throw(std::runtime_error("Dissolve is not set in this task's context.\n"));
    return dissolve_->get();
