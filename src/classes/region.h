...
class Region
{
    ...
    template <class Lam> bool generate(const Configuration *cfg, double voxelSize, Lam voxelCheckFunction)
    {
        ...
        dissolve::for_each_triplet(ParallelPolicies::par ,nVoxels_.x, nVoxels_.y, nVoxels_.z,
            [this, &voxelCheckFunction, &cfg](auto x, auto y, auto z) {
                voxelMap_[{x, y, z}] = {
                    Vec3<int>(x, y, z),
                    voxelCheckFunction(cfg, box_->getReal({(x + 0.5) * this->voxelSizeFrac_.x, (y + 0.5) * this->voxelSizeFrac_.y,
                                                               (z + 0.5) * this->voxelSizeFrac_.z}))
                    };}
        );
