...
void NETAVisitor::create(NETADefinition &neta, NETAParser::NetaContext *tree, const Forcefield *associatedForcefield)
{
    ...
    if (tree->RootNodes)
        neta_->rootNode()->setNodes(std::any_cast<NETANode::NETASequence>(visit(tree->RootNodes)));
}
...
antlrcpp::Any NETAVisitor::visitOrSequence(NETAParser::OrSequenceContext *context)
{
    ...
    orNode->setNodes(std::any_cast<NETANode::NETASequence>(LHS));

    auto RHS = visit(context->RHS);
    orNode->setAlternativeNodes(std::any_cast<NETANode::NETASequence>(RHS));
...
antlrcpp::Any NETAVisitor::visitSequence(NETAParser::SequenceContext *context)
{
    ...
    for (auto *node : context->Nodes)
        sequence.emplace_back(std::any_cast<std::shared_ptr<NETANode>>(visit(node)));
    return sequence;
}
...
antlrcpp::Any NETAVisitor::visitConnectionNode(NETAParser::ConnectionNodeContext *context)
{
    ...
    if (context->Sequence)
    {
        ...
        connectionNode->setNodes(std::any_cast<NETANode::NETASequence>(nodes));
...
antlrcpp::Any NETAVisitor::visitRingAtomNode(NETAParser::RingAtomNodeContext *context)
{
    ...
    if (context->Sequence)
    {
        ...
        auto nodes = visit(context->Sequence);
        ringAtomNode->setNodes(std::any_cast<NETANode::NETASequence>(nodes));
        contextStack_.pop_back();
    }
...
antlrcpp::Any NETAVisitor::visitRingNode(NETAParser::RingNodeContext *context)
{
    ...
    if (context->Sequence)
    {
        ...
        auto nodes = visit(context->Sequence);
        ringNode->setNodes(std::any_cast<NETANode::NETASequence>(nodes));
        contextStack_.pop_back();
    }
...
antlrcpp::Any NETAVisitor::visitSubSequence(NETAParser::SubSequenceContext *context)
{
    ...
    if (context->Sequence)
    {
        ...
        auto nodes = visit(context->Sequence);
        baseNode->setNodes(std::any_cast<NETANode::NETASequence>(nodes));
        contextStack_.pop_back();
    }
...
antlrcpp::Any NETAVisitor::visitElementOrType(NETAParser::ElementOrTypeContext *context)
{
    if (context->Element())
    {
        ...
        if (!el)
            throw(NETAExceptions::NETASyntaxException(
                std::format("'{}' is not a recognised element.\n", context->Element()->getText())));
        return el;
    }
    else if (context->FFTypeName())
    {
        ...
        if (!at)
            throw(NETAExceptions::NETASyntaxException(
                std::format("No forcefield atom type with name {} exists in forcefield '{}', so can't add it as a target.",
    ...
    else if (context->FFTypeIndex())
    {
        ...
        if (!at)
            throw(NETAExceptions::NETASyntaxException(
                std::format("No forcefield atom type with id {} exists in forcefield '{}', so can't add it as a target.", id,
...
antlrcpp::Any NETAVisitor::visitTargetList(NETAParser::TargetListContext *context, NETANode *contextNode)
{
    for (auto elementOrType : context->targets)
    {
        ...
        if (target.type() == typeid(Elements::Element))
        {
            auto Z = std::any_cast<Elements::Element>(target);
            if (!contextNode->addElementTarget(Z))
                throw(NETAExceptions::NETASyntaxException("Failed to add element to target list."));
        }
        else if (target.type() == typeid(std::reference_wrapper<const ForcefieldAtomType>))
        {
            auto atomTypeRef = std::any_cast<std::reference_wrapper<const ForcefieldAtomType>>(target);
...
void NETAVisitor::visitModifier(NETAParser::ModifierContext *context, NETANode *contextNode)
{
    ...
    if (contextNode->isValidModifier(context->Text()->getText()))
    {
        if (!contextNode->setModifier(context->Text()->getText(), op, std::stoi(context->value->getText())))
            throw(NETAExceptions::NETASyntaxException(std::format("Failed to set modifier '{}' for the current context ({}).",
                                                                  context->Text()->getText(),
                                                                  NETANode::nodeTypes().keyword(contextNode->nodeType()))));
    }
    else
        throw(NETAExceptions::NETASyntaxException(
            std::format("'{}' is not a valid modifier keyword for the current context ({}).", context->Text()->getText(),
                        NETANode::nodeTypes().keyword(contextNode->nodeType()))));
}
...
void NETAVisitor::visitOption(NETAParser::OptionContext *context, NETANode *contextNode)
{
    ...
    if (contextNode->isValidOption(context->opt->getText()))
    {
        if (!contextNode->setOption(context->opt->getText(), op, context->value->getText()))
            throw(NETAExceptions::NETASyntaxException(std::format("Failed to set option '{}' for the current context ({}).",
                                                                  context->opt->getText(),
                                                                  NETANode::nodeTypes().keyword(contextNode->nodeType()))));
    }
    else
        throw(NETAExceptions::NETASyntaxException(
            std::format("'{}' is not a valid option keyword for the current context ({}).", context->opt->getText(),
                        NETANode::nodeTypes().keyword(contextNode->nodeType()))));
}
...
void NETAVisitor::visitFlag(NETAParser::FlagContext *context, NETANode *contextNode)
{
    if (contextNode->isValidFlag(context->Text()->getText()))
    {
        if (!contextNode->setFlag(context->Text()->getText(), true))
            throw(NETAExceptions::NETASyntaxException(std::format("Failed to set flag '{}' for the current context ({}).",
                                                                  context->Text()->getText(),
                                                                  NETANode::nodeTypes().keyword(contextNode->nodeType()))));
    }
    else
        throw(NETAExceptions::NETASyntaxException(std::format("'{}' is not a valid flag for the current context ({}).",
                                                              context->Text()->getText(),
                                                              NETANode::nodeTypes().keyword(contextNode->nodeType()))));
}
...
void NETAVisitor::visitIdentifier(NETAParser::IdentifierContext *context, NETANode *contextNode)
{
    for (auto name : context->names)
    {
        if (!contextNode->addIdentifier(name->getText()))
            throw(NETAExceptions::NETASyntaxException(std::format("Failed to add identifier '{}' to the current context ({}).",
                                                                  name->getText(),
                                                                  NETANode::nodeTypes().keyword(contextNode->nodeType()))));
        else
