	...
	while (!parser.eofOrBlank())
	{
		...
		switch (kwd)
		{
			case (PairPotentialsBlock::CoulombTruncationKeyword):
				...
				{
					Messenger::error(
						"Unknown Coulomb truncation scheme '%s' used in PairPotentials block.\n",
						parser.argc(1));
					error = true;
					break;
				}
			...
			case (PairPotentialsBlock::ParametersKeyword):
				...
				{
					Messenger::error(
						"Unknown element '%s' given for atom type '%s' in PairPotentials block.\n",
			...
			case (PairPotentialsBlock::ShortRangeTruncationKeyword):
				...
				{
					Messenger::error(
						"Unknown short-range truncation scheme '%s' used in PairPotentials block.\n",
						parser.argc(1));
					error = true;
					break;
				}
