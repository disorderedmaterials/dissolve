...
void ConfigurationVectorKeyword::serialise(std::string name, SerialisedValue &target) const
{
  fromVector(data_, name, target, [](const auto &cfg) { return std::string(cfg->name()); });
}
