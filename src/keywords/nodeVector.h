...
class NodeVectorKeywordBase : public NodeKeywordUnderlay, public KeywordBase
{
    ...
    void serialise(std::string name, SerialisedValue &target) const override { throw std::runtime_error("Cannot serialise NodeVectorKeywordBase"); }
};
...
template <class N> class NodeVectorKeyword : public NodeVectorKeywordBase
{
    ...
    {
        target[name] = fromVector(data_, [](const auto &item) { return item->name(); });
    }
