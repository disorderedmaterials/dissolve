...
const SampledDouble& Sum1DProcedureNode::sum(int index) const
{
	return sum_[index];
}

// Return whether range B is enabled (from keyword data)
bool Sum1DProcedureNode::isRangeBEnabled() const
{
	return keywords_.asBool("RangeBEnabled");
}

// Return whether range C is enabled (from keyword data)
bool Sum1DProcedureNode::isRangeCEnabled() const
{
	return keywords_.asBool("RangeCEnabled");
}
...
bool Sum1DProcedureNode::prepare(Configuration* cfg, const char* prefix, GenericList& targetList)
{
...
ProcedureNode::NodeExecutionResult Sum1DProcedureNode::execute(ProcessPool& procPool, Configuration* cfg, const char* prefix, GenericList& targetList)
{
	...
	if (rangeBEnabled_) sum_[1] += Integrator::sum(processNode_->processedData(), rangeB_);
	if (rangeCEnabled_) sum_[2] += Integrator::sum(processNode_->processedData(), rangeC_);

	// Print info
	Messenger::print("Sum1D - Range A: %e +/- %e over %e < x < %e.\n", sum_[0].mean(), sum_[0].stDev(), rangeA_.minimum(), rangeA_.maximum());
	if (rangeBEnabled_) Messenger::print("Sum1D - Range B: %e +/- %e over %e < x < %e.\n", sum_[1].mean(), sum_[1].stDev(), rangeB_.minimum(), rangeB_.maximum());
	if (rangeCEnabled_) Messenger::print("Sum1D - Range C: %e +/- %e over %e < x < %e.\n", sum_[2].mean(), sum_[2].stDev(), rangeC_.minimum(), rangeC_.maximum());
