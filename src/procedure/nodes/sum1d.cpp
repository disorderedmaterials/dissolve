...
ProcedureNode::NodeExecutionResult Sum1DProcedureNode::execute(ProcessPool& procPool, Configuration* cfg, const char* prefix, GenericList& targetList)
{
	...
	// Calculate integrals
	sum_[0] += Integrator::sum(processNode_->processedData(), rangeA);
	sum_[1] += Integrator::sum(processNode_->processedData(), rangeB);
	sum_[2] += Integrator::sum(processNode_->processedData(), rangeC);

	// Print info
	Messenger::print("Sum1D - Range A: %e +/- %e over %e < x < %e.\n", sum_[0].mean(), sum_[0].stDev(), rangeA.minimum(), rangeA.maximum());
	Messenger::print("Sum1D - Range B: %e +/- %e over %e < x < %e.\n", sum_[1].mean(), sum_[1].stDev(), rangeB.minimum(), rangeB.maximum());
	Messenger::print("Sum1D - Range C: %e +/- %e over %e < x < %e.\n", sum_[2].mean(), sum_[2].stDev(), rangeC.minimum(), rangeC.maximum());
