...
bool RotateProcedureNode::execute(const ProcedureContext &procedureContext)
{
    ...
    */
    ...
    {
        Messenger::warn("Rotation can only be applied to fragment sites, site {} originates from {} which is not a fragment site, so it will be skipped.", parent->name(), parentIndex);
        return false;
    }
