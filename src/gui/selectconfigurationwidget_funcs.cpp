...
void SelectConfigurationWidget::reset(int minSize, int maxSize)
{
	...
	if (maxSize == 1) ui_.ConfigurationList->setSelectionMode(QAbstractItemView::SingleSelection);
	else ui_.ConfigurationList->setSelectionMode(QAbstractItemView::ExtendedSelection);
...
void SelectConfigurationWidget::on_SelectNoneButton_clicked(bool checked)
{
	ui_.ConfigurationList->clearSelection();
}

void SelectConfigurationWidget::on_SelectAllButton_clicked(bool checked)
{
	ui_.ConfigurationList->selectAll();
}
...
RefList<Configuration> SelectConfigurationWidget::currentConfiguration() const
{
	...
	for (int n=0; n<ui_.ConfigurationList->count(); ++n)
	{
		...
		if (item->isSelected()) selection.append(VariantPointer<Configuration>(item->data(Qt::UserRole)));
	}
