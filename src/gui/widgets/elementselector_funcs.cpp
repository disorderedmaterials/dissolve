...
// 		button->setMaximumSize(24,24);
	...
	for (n=0; n<6; n++)
	{
		for (m=0; m<6; m++) gl->addWidget(elementButtons_.item(z+m), n+2, 13+m);
		z += 8;
		if (n > 0) z += 10;
		if (n > 2) z += 14;
	}
	...
	for (n=0; n<4; n++)
	{
		for (m=0; m<10; m++) gl->addWidget(elementButtons_.item(z+m), n+4, 3+m);
		if (n == 1) z += 14;
		z += 18;
		if (n > 1) z += 14;
	}
...
Element* ElementSelector::currentElement() const
{
