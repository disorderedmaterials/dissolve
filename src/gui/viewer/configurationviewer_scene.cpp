/*
	...
	*** src/gui/viewer/configurationviewer_scene.cpp
...
void ConfigurationViewer::createPrimitives()
{
...
void ConfigurationViewer::setupGL()
{
	// Define colours etc.
 ...
 * Draw scene
 */
void ConfigurationViewer::drawScene()
{
	Matrix4 A;
	Vec3<double> v;
	...
	{
		Messenger::error("Called ConfigurationViewer::drawConfiguration() with a NULL Configuration pointer.\n");
		return;
	}
	...
	if (sourceConfiguration_->box()->type() != Box::NonPeriodicBoxType)
	{
	...
	if ((drawStyle_ == ConfigurationViewer::LineStyle) || (drawStyle_ == ConfigurationViewer::PointStyle))
	{
		...
		// Draw Atoms
		DynamicArray<Atom>& atoms = sourceConfiguration_->atoms();
		Atom* i;
...
// 			pointPrimitives_.defineVertex(ri.x, ri.y, ri.z, 0.0, 0.0, 1.0, colour[0], colour[1], colour[2], colour[3], false);
			// TODO Use proper element colours
			pointPrimitives_.defineVertex(ri.x, ri.y, ri.z, 0.0, 0.0, 1.0, TEMPORARYCOLOUR[0], TEMPORARYCOLOUR[1], TEMPORARYCOLOUR[2], TEMPORARYCOLOUR[3], false);
		}

		// Draw Bonds  TODO
// 		if (drawStyle_ == ConfigurationViewer::LineStyle) for (Bond* b = sourceConfiguration_->bonds(); b != NULL; b = b->next)
// 		{
...
// 		}
	...
	else if (drawStyle_ == ConfigurationViewer::BallAndStickStyle)
	{
		// Draw Atoms
		Isotope* iso;
		DynamicArray<Atom>& atoms = sourceConfiguration_->atoms();
...
// 			renderPrimitive(&spherePrimitive03_, PeriodicTable::element(i->element()).colour(), A);
			renderPrimitive(&spherePrimitive03_, colourBlack, A);
		}
 ...
 */
void ConfigurationViewer::setSource(Configuration* cfg)
{
 ...
 * Set draw style
 */
void ConfigurationViewer::setDrawStyle(ConfigurationViewer::DrawStyle style)
{
	drawStyle_ = style;
	update();
}
 ...
 * Change draw style
 */
void ConfigurationViewer::changeDrawStyle(int style)
{
	drawStyle_ = (ConfigurationViewer::DrawStyle) style;
	update();
}
