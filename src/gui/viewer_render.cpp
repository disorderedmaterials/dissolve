    ...
    if (axesVisible_)
    {
            ...
            for (auto axis = 0; axis < 3; ++axis)
                if (view_.axes().visible(axis) && (axis != skipAxis))
                {
                    view_.axes().labelPrimitive(axis).renderAll(fontInstance_, viewMatrix, viewRotationInverse,
                                                                view_.textZScale());
                    updateQuery(BaseViewer::AxisTickLabelObject, std::format("{}", axis), fmt::format("{}", char(88 + axis)));
                    view_.axes().titlePrimitive(axis).renderAll(fontInstance_, viewMatrix, viewRotationInverse,
                                                                view_.textZScale());
                    updateQuery(BaseViewer::AxisTitleLabelObject, std::format("{}", axis), fmt::format("{}", char(88 + axis)));
                }
        }
        ...
        for (auto axis = 0; axis < 3; ++axis)
            if (view_.axes().visible(axis) && (axis != skipAxis))
            {
                ...
                updateQuery(BaseViewer::GridLineMinorObject, std::format("{}", axis), fmt::format("{}", char(88 + axis)));
            }
        for (auto axis = 0; axis < 3; ++axis)
            if (view_.axes().visible(axis) && (axis != skipAxis))
            {
                ...
                updateQuery(BaseViewer::GridLineMajorObject, std::format("{}", axis), fmt::format("{}", char(88 + axis)));
            }
        ...
        for (auto axis = 0; axis < 3; ++axis)
            if (view_.axes().visible(axis) && (axis != skipAxis))
            {
                ...
                updateQuery(BaseViewer::AxisLineObject, std::format("{}", axis), fmt::format("{}", char(88 + axis)));
            }
