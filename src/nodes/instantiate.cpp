...
std::string_view InstantiateNode::type() const { return "InsertRandom"; };

// Return short summary of the node's purpose
std::string_view InstantiateNode::summary() const { return "Insert molecules randomly into a configuration"; };
...
{
    // Check instances
    if (!species_->structure())
        return error("Species '{}' has no instances defined for it.\n", species_->name());
    ...
    switch (boxScaling_)
    {
        case (InstantiateNode::BoxScalingType::None):
            message("No scaling will be applied to the current box.\n");
            break;
        case (InstantiateNode::BoxScalingType::AddVolume):
            ...
            break;
        case (InstantiateNode::BoxScalingType::FixedDensity):
