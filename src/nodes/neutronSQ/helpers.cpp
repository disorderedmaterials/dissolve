...
bool NeutronSQNode::calculateWeightedGR(const PartialSet &unweightedgr, PartialSet &weightedgr, NeutronWeights &weights,
...
bool NeutronSQNode::calculateWeightedSQ(const PartialSet &unweightedsq, PartialSet &weightedsq, NeutronWeights &weights,
                                        StructureFactors::NormalisationType normalisation)
{
    int typeI, typeJ;
...
void NeutronSQNode::calculateWeights(NeutronWeights &weights) const
{
    // Clear weights and get species speciesPopulations_ from GRModule
    weights.clear();

    for (auto &[sp, pop] : unweightedGR_->realSpeciesPopulations())
    {
