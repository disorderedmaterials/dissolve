...
{
    // Inputs
    addInput("Configuration", "Set target configuration for the module", targetConfiguration_)       ->setFlags({ParameterBase::Required});
    addInput("Temperature", "Temperature (K)", temperature_)->setFlags({ParameterBase::Required, ParameterBase::ClearData});

    // Options
    addOption("ShakesPerMolecule", "Number of shakes to attempt per molecule", nShakesPerMolecule_);
    ...
    addOption("TranslationStepSizeMin", "Minimum step size for translations (Angstroms)",
                                 translationStepSizeMin_);
    addOption("TranslationStepSizeMax", "Maximum step size for translations (Angstroms)",
                                 translationStepSizeMax_);
    // Outputs
...
std::string_view MCNode::summary() const
{
    return "Perform standard Monte Carlo evolution on the target configuration";
}
