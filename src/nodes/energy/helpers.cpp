...
double EnergyNode::interMolecularEnergy(const ProcessPool& procPool, const Configuration* cfg,
    const PotentialMap& potentialMap)
{
    /*
...
double EnergyNode::intraMolecularEnergy(const ProcessPool& procPool, const Configuration* cfg,
    const PotentialMap& potentialMap)
{
...
EnergyNode::EnergyStability EnergyNode::checkStability(GenericList& processingData, const Configuration* cfg)
{
    // First, check if the Configuration is targetted by an EnergyNode
    /*
    if (!processingData.valueOr<bool>("IsEnergyModuleTarget", cfg->niceName(), false))
    {
        Messenger::error("Configuration '{}' is not targeted by any EnergyNode, so stability cannot be assessed. "
            "Check your setup!\n",
            cfg->name());
        return EnergyNode::NotAssessable;
    }
    */
    ...
    if (processingData.contains("EnergyStable", cfg->niceName()))
    {
        ...
        if (!stable)
        {
            message("Energy for Configuration '{}' is not yet stable.\n", cfg->name());
            return EnergyNode::EnergyUnstable;
        }
    }
    else
    {
        warn("No energy stability information is present for Configuration '{}' (yet) - check your setup.\n",
            cfg->name());
        return EnergyNode::NotAssessable;
    }
    */
    return EnergyNode::EnergyStable;
}

// Check energy stability of specified Configurations, returning the number that ExecutionResult::Failed
int EnergyNode::nUnstable(GenericList& processingData, const std::vector<Configuration*>& configurations)
{
    auto nFailed = 0;
    ...
    for (auto* cfg : configurations)
    {
        ...
        if (result == EnergyNode::EnergyStable)
            ++nFailed;
        else if (result == EnergyNode::NotAssessable)
            return EnergyNode::NotAssessable;
    }
