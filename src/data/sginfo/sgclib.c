...
static const char *Err_Ill_SMx_in_List = "Error: Illegal SeitzMx in list";
...
static void CopyRotMxInfo(T_RotMxInfo *target, const T_RotMxInfo *source) { memcpy(target, source, sizeof(*target)); }
...
int traceRotMx(const int *RotMx) { return RotMx[0] + RotMx[4] + RotMx[8]; }
...
static int nNextBasis_of_DirCode(const int DirCode, const int **RMx, const int **InvRMx)
{
    switch (DirCode)
    {
        case '.':
...
static int CoreAdd2ListSeitzMx(T_SgInfo *SgInfo, const T_RTMx *NewSMx)
{
    ...
    static const char *Err_NonXtalOp = "Error: Generators produce non-crystallographic operation";
...
int Add2ListSeitzMx(T_SgInfo *SgInfo, const T_RTMx *NewSMx)
{
    ...
    static const char *Err_Ill_lattice_translation = "Error: Illegal lattice translation";
...
static void TidyTranslation(T_SgInfo *SgInfo)
{
    ...
    for (iList = 0; iList < SgInfo->nList; iList++, lsmx++)
    {
        ...
        for (iTrV = 0; iTrV < nTrV; iTrV++)
        {
            ...
            if (n0t > n0mint ||
                (n0t == n0mint && (t1 + t2 + t3 < mint1 + mint2 + mint3 ||
                                   (t1 + t2 + t3 == mint1 + mint2 + mint3 && (t1 < mint1 || (t1 == mint1 && t2 < mint2))))))
            {
                mint1 = t1;
                mint2 = t2;
                mint3 = t3;
...
static int SgInfoListSortFunction(const int *iList_a, const int *iList_b)
{
    ...
    if (SgInfo->ListRotMxInfo == NULL)
    {
        ...
        if (GetRotMxInfo(smx_a->s.R, rmxi_a) == 0 || GetRotMxInfo(smx_b->s.R, rmxi_b) == 0)
        {
            SetSgError(Err_Ill_SMx_in_List);
            return 0;
        }
...
static void SortSgInfoList(T_SgInfo *SgInfo, int *List_iList)
{
    ...
    qsort((void *)List_iList, nList, sizeof(*List_iList), (int (*)(const void *, const void *))SgInfoListSortFunction);
...
int FindSeitzMx(const T_SgInfo *SgInfo, int Order, int HonorSign, int RefAxis, int DirCode)
{
    ...
    for (iList = 0; iList < SgInfo->nList; iList++, lsmx++)
    {
            ...
            (DirCode == 0 || DirCode == lrmxi->DirCode))
        {
            ...
            if (lrmxi->EigenVector[0] == 1 && lrmxi->EigenVector[1] == 1 && lrmxi->EigenVector[2] == 1)
                return iList;
        }
...
int CompleteSgInfo(T_SgInfo *SgInfo)
{
    ...
    if (SgInfo->TabSgName != NULL && tsgn->HallSymbol == NULL)
    {
        ...
        sprintf(SgErrorBuffer, "Internal Error: Input/Output HallSymbol mismatch: %s <> %s", SgInfo->TabSgName->HallSymbol,
                SgInfo->HallSymbol);
