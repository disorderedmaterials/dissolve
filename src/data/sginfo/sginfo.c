...
static int Try_GS_si(T_SgInfo *SgInfo)
{
    ...
    nProperty = (Maxh - Minh + 1) * (Maxk - Mink + 1) * (Maxl - Minl + 1);
...
static void NotEnoughCore(void) { progerror("Not enough core"); }
...
static void PutAllXYZ(const T_SgInfo *SgInfo, FILE *fpout)
{
    ...
    for (iTrV = 0; iTrV < nTrV; iTrV++, TrV += 3)
    {
        for (iLoopInv = 0; iLoopInv < nLoopInv; iLoopInv++)
        {
            ...
            if (nTrV > 1)
                fprintf(fpout, " +(%s %s %s)", FormatFraction(TrV[0], STBF, 0, buf0, sizeof buf0 / sizeof(*buf0)),
                ...
                if (SuppressMx == 0)
                {
                    for (i = 0; i < 3; i++)
                        fprintf(fpout, " %12.8f %12.8f %12.8f %12.8f\n", (double)f * lsmx->s.R[3 * i + 0],
                                (double)f * lsmx->s.R[3 * i + 1], (double)f * lsmx->s.R[3 * i + 2],
...
static void Simple_hklList(T_SgInfo *SgInfo, int Maxh, int Maxk, int Maxl, int ListSysAbsent)
{
    ...
    for (h = Minh; h <= Maxh; h++)
        for (k = Mink; k <= Maxk; k++)
            for (l = Minl; l <= Maxl; l++)
            {
                ...
                if (iList == 0)
                {
                    if ((iList = IsSuppressed_hkl(SgInfo, Minh, Mink, Minl, Maxk, Maxl, h, k, l)) != 0)
                        n = fprintf(stdout, "# %3d %3d %3d  %3d  [%d]", h, k, l, M, iList);
                    else
...
static int Lc2RLc(T_LatticeConstants *lc, T_LatticeConstants *rlc)
{
    ...
    D = 1. - lc->ca * lc->ca - lc->cb * lc->cb - lc->cg * lc->cg + 2. * lc->ca * lc->cb * lc->cg;
    if (D < 0.)
        return -1;
...
IntErr:
    SetSgError("Internal Error: HarmonizeSgLatCon()");
    return -1;

IllUnitCell:
    SetSgError("Error: Illegal UnitCell or SpaceGroup");
    return -1;
}
...
{
    static const char *quick_help[] = {"-Hall|VolA|VolI   select conventions",
    ...
    fprintf(stderr, "usage: %s [options] [SpaceGroupName_or_# [SpaceGroupName_or_#]]\n", progn);
 ...
 */
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        else if (str_icmp(argv[i], "-VolI") == 0 || str_icmp(argv[i], "-Vol1") == 0)
        {
            F_Convention = 'I';
            Last_F_Convention = 'I';
        }
        ...
        {
            F_UnitCell = sscanf(&argv[i][10], "%lf%lf%lf%lf%lf%lf", &LatConA.a, &LatConA.b, &LatConA.c, &LatConA.alpha,
                                &LatConA.beta, &LatConA.gamma);
    ...
    for (iSgList = 0; iSgList < nSgList; iSgList++)
    {
        ...
        SgInfo->ListRotMxInfo = malloc(SgInfo->MaxList * sizeof(*SgInfo->ListRotMxInfo));
        ...
        if (nSgList > 1 || F_Standard)
        {
            ...
            SgList[iSgList].RefTSgN = FindReferenceSpaceGroup(SgInfo, CBMx, InvCBMx);
            PrintClearSgError(F_ClearError, 0);
    ...
    if (nSgList == 2 && SgList[0].RefTSgN && SgList[1].RefTSgN && SgList[0].RefTSgN->SgNumber == SgList[1].RefTSgN->SgNumber)
    {
        ...
        RTMxMultiply(&CCBMx, &SgList[1].InvCBMx, &SgList[0].CBMx, CRBF, CRBF * CTBF);
        RTMxMultiply(&CInvCBMx, &SgList[0].InvCBMx, &SgList[1].CBMx, CRBF, CRBF * CTBF);
        ...
        else
        {
            ...
            if (F_UnitCell)
            {
                ...
                fprintf(stdout, "Setting A UnitCell  %.6g %.6g %.6g %.6g %.6g %.6g\n", LatConA.a, LatConA.b, LatConA.c,
                        LatConA.alpha / PIover180, LatConA.beta / PIover180, LatConA.gamma / PIover180);

                fprintf(stdout, "Setting B UnitCell  %.6g %.6g %.6g %.6g %.6g %.6g\n", LatConB.a, LatConB.b, LatConB.c,
                        LatConB.alpha / PIover180, LatConB.beta / PIover180, LatConB.gamma / PIover180);
            }
        }
