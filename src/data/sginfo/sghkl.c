...
static const char *IErr_Inc_SymMx = "Internal Error: Inconsistent symmetry matrices";
...
int IsSuppressed_hkl(const T_SgInfo *SgInfo, int Minh, int Mink, int Minl, int Maxk, int Maxl, int h, int k, int l)
{
    ...
    if ((Minh <= hm && hm <= h) && (Mink <= km && km <= Maxk) && (Minl <= lm && lm <= Maxl))
    { /* TGAY Added explicit braces to avoid dangling else 11/2019 */
        if (hm < h)
            return -1;
