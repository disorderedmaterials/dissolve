...
bool Forcefield_UFF4MOF::setUp()
{
    uffAtomTypes_ = {
        ...
        {Elements::O, 202, "O_3_f", "", "Oxygen (tetrahedral) in framework materials", 0.634, 109.47, 3.5, 0.06, 14.085 , 2.3, 8.741, 0.018, 2.0},
        {Elements::O, 203, "O_2_z", "", "Oxygen (trigonal planar) in SBU", 0.528, 120.0, 3.5, 0.06, 14.085, 2.3, 8.741, 0.0, 2.0},
        {Elements::Al, 204, "Al6+3", "", "Aluminium (octahedral, oxidation state = +3)", 1.22, 90.0, 4.499, 0.505, 11.278, 1.792, 11.278, 0.0, 1.25},
        {Elements::Sc, 205, "Sc6+3", "", "Scandium (octahedral, oxidation state = +3)", 1.44, 90.0, 3.295, 0.019, 12.0, 2.595, 3.395, 0.0, 0.7},
        {Elements::Ti, 206, "Ti4+2", "", "Titanium (square planar, oxidation state = +2)", 1.38, 90.0, 3.175, 0.017, 12.0, 2.659, 3.47, 0.0, 0.7},
        {Elements::V, 207, "V_4+2", "", "Vanadium (square planar, oxidation state = +2)", 1.18, 90.0, 3.144, 0.016, 12.0, 2.679, 3.65, 0.0, 0.7},
        {Elements::V, 208, "V_6+3", "", "Vanadium (octahedral, oxidation state = +3)", 1.30, 90.0, 3.144, 0.016, 12.0, 2.679, 3.65, 0.0, 0.7},
        {Elements::Cr, 209, "Cr4+2", "", "Chromium (square planar, oxidation state = +2)", 1.10, 90.0, 3.023, 0.015, 12.0, 2.463, 3.415, 0.0, 0.7},
        {Elements::Cr, 210, "Cr6f3", "", "Chromium (octahedral, oxidation state = +3)", 1.28, 90.0, 3.023, 0.015, 12.0, 2.463, 3.415, 0.0, 0.7},
        {Elements::Mn, 211, "Mn6+3", "", "Manganese (octahedral, oxidation state = +3)", 1.34, 90.0, 2.961, 0.013, 12.0, 2.43, 3.325, 0.0, 0.7},
    ...
    auto nFailed = std::count_if(uffAtomTypes_.begin(), uffAtomTypes_.end(), [this](auto &atomType) {
        ...
        if (!success)
            Messenger::print("Failed to parse NETA definition '{}' for UFF4MOF atom type '{}'.", atomType.neta().definitionString(),
                             atomType.name());
        return !success;
    });
