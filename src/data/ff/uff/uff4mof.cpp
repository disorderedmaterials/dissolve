...
bool Forcefield_UFF4MOF::setUp()
{
    uffAtomTypes_ = {
        ...
        {Elements::O, 202, "O_3_f", "nbonds=4, -219", "Oxygen (tetrahedral) in framework materials", 0.634, 109.47, 3.5, 0.06, 14.085, 2.3,
         8.741, 0.018, 2.0},
        {Elements::O, 203, "O_2_z", "nbonds=3, -[204,205,208,211,213,210]", "Oxygen (trigonal planar) in SBU", 0.528, 120.0, 3.5, 0.06, 14.085, 2.3, 8.741, 0.0,
         2.0},
        {Elements::Al, 204, "Al6+3", "nbonds=6, -203", "Aluminium (octahedral, oxidation state = +3)", 1.22, 90.0, 4.499, 0.505, 11.278,
         1.792, 11.278, 0.0, 1.25},
        {Elements::Sc, 205, "Sc6+3", "nbonds=6, -203", "Scandium (octahedral, oxidation state = +3)", 1.44, 90.0, 3.295, 0.019, 12.0, 2.595,
         3.395, 0.0, 0.7},
        {Elements::Ti, 206, "Ti4+2", "nbonds=4, -O(-9)", "Titanium (square planar, oxidation state = +2)", 1.38, 90.0, 3.175, 0.017, 12.0,
         2.659, 3.47, 0.0, 0.7},
        {Elements::V, 207, "V_4+2", "nbonds=4", "Vanadium (square planar, oxidation state = +2)", 1.18, 90.0, 3.144, 0.016, 12.0, 2.679,
         3.65, 0.0, 0.7},
        {Elements::V, 208, "V_6+3", "nbonds=6, -203", "Vanadium (octahedral, oxidation state = +3)", 1.30, 90.0, 3.144, 0.016, 12.0, 2.679,
         3.65, 0.0, 0.7},
        {Elements::Cr, 209, "Cr4+2", "nbonds=4, -O(-9)", "Chromium (square planar, oxidation state = +2)", 1.10, 90.0, 3.023, 0.015, 12.0,
         2.463, 3.415, 0.0, 0.7},
        {Elements::Cr, 210, "Cr6f3", "nbonds=6, -203", "Chromium (octahedral, oxidation state = +3)", 1.28, 90.0, 3.023, 0.015, 12.0, 2.463,
         3.415, 0.0, 0.7},
        {Elements::Mn, 211, "Mn6+3", "nbonds=6, -203", "Manganese (octahedral, oxidation state = +3)", 1.34, 90.0, 2.961, 0.013, 12.0, 2.43,
         3.325, 0.0, 0.7},
        {Elements::Mn, 212, "Mn4+2", "nbonds=4, -O(-9)", "Mangansese (square planar, oxidation state = +2)", 1.26, 90.0, 2.961, 0.013, 12.0,
         3.325, 0.0, 0.7},
        {Elements::Fe, 213, "Fe6+3", "nbonds=6, -203", "Iron (octahedral, oxidation state = +3)", 1.32, 90.0, 2.912, 0.013, 12.0, 2.43, 3.76,
         0.0, 0.7},
        {Elements::Fe, 214, "Fe4+2", "nbonds=4, -O(-9)", "Iron (square planar, oxidation state = +2)", 1.10, 90.0, 2.912, 0.013, 12.0, 2.43,
         3.76, 0.0, 0.7},
        {Elements::Co, 215, "Co3+2", "nbonds=4", "Cobalt (tetrahedral, oxidation state = +2)", 1.24, 109.47, 2.872, 0.014, 12.0, 1.308,
         4.105, 0.0, 0.7},
        {Elements::Co, 216, "Co4+2", "nbonds=4, -[O(-9),13]", "Cobalt (square planar, oxidation state = +3)", 1.16, 90.0, 2.872, 0.014, 12.0, 1.308,
         4.105, 0.0, 0.7},
        {Elements::Cu, 217, "Cu4+2", "nbonds=4, -O(-9)", "Copper (square planar, oxidation state = +2)", 1.28, 90.0, 3.495, 0.005, 12.0, 2.43,
         4.2, 0.0, 0.7},
        {Elements::Zn, 218, "Zn4+2", "nbonds=4, -[O(-9),13]", "Zinc (square planar, oxidation state = +2)", 1.34, 90.0, 2.763, 0.124, 12.0, 1.308,
         5.106, 0.0, 0.7},
        {Elements::Zn, 219, "Zn3f2", "-[202,13]", "Zinc (tetrahedral, oxidation state = +2)", 1.24, 109.47, 2.763, 0.124, 12.0, 1.308,
         5.106, 0.0, 0.7}
...
Forcefield::ShortRangeType Forcefield_UFF4MOF::shortRangeType() const { return Forcefield::LennardJonesType; }
...
OptionalReferenceWrapper<const UFFAtomType> Forcefield_UFF4MOF::uffAtomTypeByName(std::string_view name) const
{
...
OptionalReferenceWrapper<const UFFAtomType> Forcefield_UFF4MOF::uffAtomTypeForElement(int el) const
{
...
OptionalReferenceWrapper<const UFFAtomType> Forcefield_UFF4MOF::determineUFFAtomType(const SpeciesAtom *i) const
{
...
// Determine and return atom type for specified SpeciesAtom
OptionalReferenceWrapper<const ForcefieldAtomType> Forcefield_UFF4MOF::determineAtomType(const SpeciesAtom &i) const
{
...
double Forcefield_UFF4MOF::bondOrder(const UFFAtomType &i, const UFFAtomType &j) const
{
...
double Forcefield_UFF4MOF::bondOrderCorrection(const UFFAtomType &i, const UFFAtomType &j) const
{
...
double Forcefield_UFF4MOF::electronegativityCorrection(const UFFAtomType &i, const UFFAtomType &j) const
{
...
bool Forcefield_UFF4MOF::assignBondTermParameters(SpeciesBond &bond, bool determineTypes) const
{
...
bool Forcefield_UFF4MOF::assignAngleTermParameters(SpeciesAngle &angle, bool determineTypes) const
{
    ...
    if (geom == '1')
        angle.setParameters({4.184 * kijk, 1.0, 0.0, 1.0});
    else if (geom == '2')
        angle.setParameters({4.184 * kijk / 9.0, 3.0, 0.0, -1.0});
    else if (geom == '3' && fabs(90.0 - j.theta()) < 0.1)
        angle.setParameters({4.184 * kijk / 4.0, 2.0, 0.0, 1.0});
    else if (geom == '4' || geom == '6')
        angle.setParameters({4.184 * kijk / 16.0, 4.0, 0.0, -1.0});
    else
    {
...
bool Forcefield_UFF4MOF::assignTorsionTermParameters(SpeciesTorsion &torsion, bool determineTypes) const
{
