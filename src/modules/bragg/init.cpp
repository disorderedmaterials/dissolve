...
void BraggModule::initialise()
{
	...
	keywords_.add("Calculation", new EnumOptionsKeyword<Averaging::AveragingScheme>(Averaging::averagingSchemes() = Averaging::LinearAveraging), "AveragingScheme",
		      "Weighting scheme to use when averaging reflection data", "<Linear>");
	...
	keywords_.add("Calculation", new Vec3IntegerKeyword(Vec3<int>(1, 1, 1), Vec3<int>(1, 1, 1), Vec3Labels::MinMaxDeltaLabels), "Multiplicity",
		      "Bragg intensity scaling factor accounting for number of repeat units in Configuration", "<1 1 1>");
