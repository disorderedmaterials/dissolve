...
void BraggModule::initialise()
{
	...
	keywords_.add("Calculation", new DoubleKeyword(0.001), "QDelta",
	...
	keywords_.add("Calculation",
		      new Vec3IntegerKeyword(Vec3<int>(1, 1, 1), Vec3<int>(1, 1, 1), Vec3Labels::MinMaxDeltaLabels),
		      "Multiplicity", "Bragg intensity scaling factor accounting for number of repeat units in Configuration",
		      "<1 1 1>");

	// Export
	keywords_.add("Export", new BoolKeyword(false), "SavePartials",
		      "Whether to save Bragg partials to disk after calculation", "<True|False>");
