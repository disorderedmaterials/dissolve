...
bool ChecksModule::process(DUQ& duq, ProcessPool& procPool)
{
	...
	for (RefListItem<Configuration,bool>* ri = targetConfigurations_.first(); ri != NULL; ri = ri->next)
	{
		...
		const double angleThreshold = GenericListHelper<double>::retrieve(moduleData, "AngleThreshold", uniqueName(), options_.valueAsDouble("AngleThreshold"));
		const double distanceThreshold = GenericListHelper<double>::retrieve(moduleData, "DistanceThreshold", uniqueName(), options_.valueAsDouble("DistanceThreshold"));
