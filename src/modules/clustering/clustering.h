...
class ClusteringModule : public Module
{
    ...
    double cutoff_{3.0};
    // Fisher exponent for percolation line
    double fisher_{-2.189};
    ...
    // Int for how many molecules are not in clusters
    int molsNotClustered_ = 0;
    ...
    // {Site A : {Site B, C ... : Contact Number for each pair}} where Site A - B, C ... are all the combinations defined
