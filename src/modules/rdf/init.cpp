...
{
	static EnumOptionsList PartialsMethodOptions =
		EnumOptionsList() << EnumOption(RDFModule::AutoMethod, "Auto") << EnumOption(RDFModule::CellsMethod, "Cells")
...
void RDFModule::initialise()
{
	...
	keywords_.add("Test", new BoolKeyword(false), "Test",
		      "Test calculated total and partials against supplied reference data", "<True|False>");
	...
	keywords_.add("Test", new DoubleKeyword(0.1, 1.0e-5), "TestThreshold",
		      "Test threshold (%%error) above which test fails", "<threshold[0.1]>");
