...
// SQ Module
class CGXRaySQModule : public Module
{
    public:
    CGXRaySQModule();
    ~CGXRaySQModule() override = default;
    ...
    bool calculateWeightedGR(const PartialSet &unweightedgr, PartialSet &weightedgr, const CGXRayWeights &weights,
                             StructureFactors::NormalisationType normalisation);
    // Calculate weighted S(Q) from supplied unweighted S(Q) and Weights
    bool calculateWeightedSQ(const PartialSet &unweightedsq, PartialSet &weightedsq, const CGXRayWeights &weights,
                             StructureFactors::NormalisationType normalisation);
    // Calculate xray weights for relevant Configuration targets
    void calculateWeights(const GRModule *rdfModule, CGXRayWeights &weights,
