...
// Set target data
void CGXRaySQModule::setTargets(const std::vector<std::unique_ptr<Configuration>> &configurations,
...
// Run set-up stage
bool CGXRaySQModule::setUp(ModuleContext &moduleContext, Flags<KeywordBase::KeywordSignal> actionSignals)
{
    /*
...
// Run main processing
Module::ExecutionResult CGXRaySQModule::process(ModuleContext &moduleContext)
{
    /*
    ...
    Messenger::print("CG XRaySQ: Source unweighted S(Q) will be taken from module '{}'.\n", sourceSQ_->name());
    if (XRayFormFactors::xRayFormFactorData().keyword(formFactors_) != "WK1995")
    {
        Messenger::print("CG XRaySQ: Requested form factors are not supported for CG calcatulions, defaulting to 'WK1995'.");
    }
    else
    {
        Messenger::print("CG XRaySQ: Form factors to use are '{}'.\n",
                         XRayFormFactors::xRayFormFactorData().keyword(formFactors_));
    }
    if (normaliseTo_ == StructureFactors::NoNormalisation)
        Messenger::print("CG XRaySQ: No normalisation will be applied to total F(Q).\n");
    else if (normaliseTo_ == StructureFactors::AverageOfSquaresNormalisation)
        Messenger::print("CG XRaySQ: Total F(Q) will be normalised to <b**2>");
    else if (normaliseTo_ == StructureFactors::SquareOfAverageNormalisation)
        Messenger::print("CG XRaySQ: Total F(Q) will be normalised to <b>**2");
    if (referenceWindowFunction_ == WindowFunction::Form::None)
        Messenger::print("CG XRaySQ: No window function will be applied when calculating representative g(r) from S(Q).");
    else
        Messenger::print("CG XRaySQ: Window function to be applied when calculating representative g(r) from S(Q) is {}.",
                         WindowFunction::forms().keyword(referenceWindowFunction_));
    if (saveFormFactors_)
        Messenger::print("CG XRaySQ: Combined form factor weightings for atomtype pairs will be saved.\n");
    if (saveSQ_)
        Messenger::print("CG XRaySQ: Weighted partial S(Q) and total F(Q) will be saved.\n");
    if (saveGR_)
        Messenger::print("CG XRaySQ: Weighted partial g(r) and total G(r) will be saved.\n");
    if (saveRepresentativeGR_)
        Messenger::print("CG XRaySQ: Representative G(r) will be saved.\n");
    Messenger::print("\n");
    ...
    weights.beadMap().calculateFormFactors(unweightedSQ.boundPartial(0, 0).xAxis());
