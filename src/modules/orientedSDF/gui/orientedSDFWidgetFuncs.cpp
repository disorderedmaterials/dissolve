...
OrientedSDFModuleWidget::OrientedSDFModuleWidget(QWidget *parent, OrientedSDFModule *module, Dissolve &dissolve)
...
void OrientedSDFModuleWidget::updateControls(const Flags<ModuleWidget::UpdateFlags> &updateFlags)
{
    refreshing_ = true;
    ...
    auto avgMolModules = Module::allOfType<AvgMolModule>(dissolve_.coreData());
...
void OrientedSDFModuleWidget::on_LowerCutoffSpin_valueChanged(double value)
{
...
void OrientedSDFModuleWidget::on_UpperCutoffSpin_valueChanged(double value)
{
    if (refreshing_ || !sdfRenderable_)
        return;
...
void OrientedSDFModuleWidget::on_ReferenceMoleculeCombo_currentIndexChanged(int index)
{
    if (refreshing_)
        return;
