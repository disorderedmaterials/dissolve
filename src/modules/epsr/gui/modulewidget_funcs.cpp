...
void EPSRModuleWidget::updateDebugEPFunctionsGraph(int from, int to)
{
    ...
    auto i = 0;
    for (std::shared_ptr<AtomType> at1 = dissolve_.atomTypes().first(); at1 != NULL; at1 = at1->next(), ++i)
    {
        auto j = i;
        for (std::shared_ptr<AtomType> at2 = at1; at2 != NULL; at2 = at2->next(), ++j)
        {
            ...
            Renderable *phi = viewer->createRenderable(
                Renderable::Data1DRenderable, CharString("PairPotential//%s//Additional", id.get()), id.get(), id.get());
