...
Module::ExecutionResult TRModule::process(ModuleContext &moduleContext)
{
    ...
    * /
    ...
    dissolve::for_each_pair(
        ...
        {
            broadenedTR.boundPartial(typeI, typeJ).copyArrays(weightedSQ.boundPartial(typeI, typeJ));
            broadenedTR.unboundPartial(typeI, typeJ).copyArrays(weightedSQ.unboundPartial(typeI, typeJ));
            broadenedTR.partial(typeI, typeJ).copyArrays(weightedSQ.partial(typeI, typeJ));

            Fourier::sineFT(broadenedTR.boundPartial(typeI, typeJ), 1.0, qMin_, qDelta_, qMax_, windowFunction_, qBroadening_);
            Fourier::sineFT(broadenedTR.unboundPartial(typeI, typeJ), 1.0, qMin_, qDelta_, qMax_, windowFunction_,
                            qBroadening_);
            Fourier::sineFT(broadenedTR.partial(typeI, typeJ), 1.0, qMin_, qDelta_, qMax_, windowFunction_, qBroadening_);

            auto factor = 4.0 * PI * rho.value() * weights.boundCoherentSquareOfAverage();
