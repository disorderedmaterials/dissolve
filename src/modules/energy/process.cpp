...
bool EnergyModule::setUp(Dissolve &dissolve, ProcessPool &procPool)
{
	...
	for (Configuration *cfg : targetConfigurations_)
		GenericListHelper<bool>::realise(cfg->moduleData(), "_IsEnergyModuleTarget", "", GenericItem::ProtectedFlag) =
			true;
...
bool EnergyModule::process(Dissolve &dissolve, ProcessPool &procPool)
{
	...
	for (RefListItem<Configuration> *ri = targetConfigurations_.first(); ri != NULL; ri = ri->next())
	{
		...
		if (testMode)
		{
			...
			if (hasReferenceInter)
			{
				...
				Messenger::print("Reference interatomic energy delta with correct value is %15.9e kJ/mol and "
						 "is %s (threshold is %10.3e kJ/mol)\n",
				...
				Messenger::print("Reference interatomic energy delta with production value is %15.9e kJ/mol "
						 "and is %s (threshold is %10.3e kJ/mol)\n",
						 delta, fabs(delta) < testThreshold ? "OK" : "NOT OK", testThreshold);
				if (!procPool.allTrue(fabs(delta) < testThreshold))
					return false;
			}
			...
			Messenger::print(
				"Intramolecular energy delta is %15.9e kJ/mol and is %s (threshold is %10.3e kJ/mol)\n",
		...
		else
		{
			...
			double intraEnergy = intraMolecularEnergy(procPool, cfg, dissolve.potentialMap(), bondEnergy,
								  angleEnergy, torsionEnergy);
			intraTimer.stop();
			...
			Data1D &interData = GenericListHelper<Data1D>::realise(cfg->moduleData(), "Inter", uniqueName(),
			...
			Data1D &totalEnergyArray = GenericListHelper<Data1D>::realise(cfg->moduleData(), "Total", uniqueName(),
										      GenericItem::InRestartFileFlag);
			...
			else
			{
				...
				Messenger::print("Gradient of last %i points is %e kJ/mol/step (absolute threshold value is "
						 "%e, stable = %s).\n",
						 stabilityWindow, grad, thresholdValue, DissolveSys::btoa(stable));
			}

			// Set variable in Configuration
			GenericListHelper<double>::realise(cfg->moduleData(), "EnergyGradient", "",
							   GenericItem::InRestartFileFlag) = grad;
			GenericListHelper<bool>::realise(cfg->moduleData(), "EnergyStable", "",
							 GenericItem::InRestartFileFlag) = stable;
			GenericListHelper<Data1D>::realise(cfg->moduleData(), "EnergyStability", "",
							   GenericItem::InRestartFileFlag)
			...
			if (saveData)
			{
				...
				parser.writeLineF("  %10i  %12.6e  %12.6e  %12.6e  %12.6e  %12.6e  %12.6e  %i\n",
						  dissolve.iteration(), interEnergy + intraEnergy, interEnergy, bondEnergy,
						  angleEnergy, torsionEnergy, grad, stable);
				parser.closeFiles();
			}
		}
