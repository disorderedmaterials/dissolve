...
bool RefineModule::process(Dissolve &dissolve, ProcessPool &procPool)
{
	 ...
	 * Get Keyword Options
	 */
	const RefineModule::MatrixAugmentationStyle augmentationStyle =
		keywords_.enumeration<RefineModule::MatrixAugmentationStyle>("Augmentation");
	...
	const RefineModule::PotentialInversionMethod inversionMethod =
		keywords_.enumeration<RefineModule::PotentialInversionMethod>("InversionMethod");
	...
	if (augmentationStyle == RefineModule::NoAugmentation)
		...
		Messenger::print(
			"Refine: Augmentation of scattering matrix will be performed (style = '%s', parameter = %e).\n",
	...
	if (autoMinimumRadii)
		...
		Messenger::print("Refine: Minimum radius of %f Angstroms will be applied to all generated potentials.\n",
				 globalMinimumRadius);
	if (smoothPhiR)
		Messenger::print("Refine: Generated phi(r) will be smoothed using KZ filter (k = %i, m = %i).\n", phiRSmoothK,
				 phiRSmoothM);
	...
	if (onlyWhenErrorStable)
		Messenger::print("Refine: Potential refinement will only be performed if all percentage errors with reference "
				 "data are stable.\n");
	if (phiMax >= 0)
		Messenger::print("Refine: Limit of additional potential for any one pair potential is %f kJ/mol/Angstrom.\n",
				 phiMax);
	else
	...
	if (windowFunction.function() == WindowFunction::NoWindow)
		...
		Messenger::print("Refine: Window function to be applied in Fourier transforms of S(Q) to g(r) is %s (%s).",
				 WindowFunction::functionType(windowFunction.function()),
				 windowFunction.parameterSummary().get());
	Messenger::print("\n");
	...
	while (ModuleGroup *group = groupIterator.iterate())
	{
		...
		for (Module *module : group->modules())
		{
			...
			Data1D &errors = GenericListHelper<Data1D>::realise(dissolve.processingModuleData(),
									    CharString("Error_%s", module->uniqueName()),
			...
			if (DissolveSys::sameString(module->type(), "NeutronSQ"))
			{
				...
				const Data1D &referenceData =
					GenericListHelper<Data1D>::value(dissolve.processingModuleData(), "ReferenceData",
				...
				const PartialSet &calcSQ =
					GenericListHelper<PartialSet>::value(dissolve.processingModuleData(), "WeightedSQ",
									     module->uniqueName(), PartialSet(), &found);
				if (!found)
				{
					Messenger::warn(
						"Could not locate associated weighted neutron PartialSet for target '%s'.\n",
						module->uniqueName());
					return false;
				}
				...
				Data1D &differenceData = GenericListHelper<Data1D>::realise(
					dissolve.processingModuleData(), CharString("DifferenceData_%s", module->uniqueName()),
					uniqueName());
				differenceData.setObjectTag(
					CharString("%s//Difference//%s", uniqueName_.get(), module->uniqueName()));
				differenceData = referenceData;
				Interpolator::addInterpolated(differenceData, calcSQTotal, -1.0);
			}
			else
				return Messenger::error("Unrecognised Module type '%s', so can't calculate error.",
							module->type());
			...
			else
			{
				...
				Messenger::print("Error gradient of last %i points for reference data '%s' is %e %%/step "
						 "(absolute threshold value is %e, stable = %s).\n",
	...
	{
		Messenger::print("Errors for %i reference datasets are unstable, so no potential refinement will be performed "
				 "this iteration.\n",
				 nUnstableData);
		return true;
	}
	...
	while (ModuleGroup *group = groupIterator.iterate())
	{
		...
		Array2D<Data1D> &combinedUnweightedSQ = GenericListHelper<Array2D<Data1D>>::realise(
		...
		for (AtomType *at1 = dissolve.atomTypes().first(); at1 != NULL; at1 = at1->next(), ++i)
		{
			...
			for (AtomType *at2 = at1; at2 != NULL; at2 = at2->next(), ++j)
				combinedUnweightedSQ.at(i, j).setObjectTag(CharString(
					"%s//UnweightedSQ//%s//%s-%s", uniqueName(), group->name(), at1->name(), at2->name()));
		}

		// Realise storage for generated S(Q), and reinitialise the scattering matrix
		Array2D<Data1D> &estimatedSQ = GenericListHelper<Array2D<Data1D>>::realise(
			dissolve.processingModuleData(), "EstimatedSQ", uniqueName_, GenericItem::InRestartFileFlag);
		...
		for (Module *module : targetModules)
		{
			...
			if (!found)
				return Messenger::error("Could not locate UnweightedSQ for target '%s'.\n",
							module->uniqueName());
			double rho = GenericListHelper<double>::value(dissolve.processingModuleData(), "EffectiveRho",
								      module->uniqueName(), 0.0, &found);
			if (!found)
			...
			if (!scatteringMatrix_.addReferenceData(referenceData, weights, dataFactor))
				return Messenger::error("Failed to add target data '%s' to weights matrix.\n",
							module->uniqueName());
			...
			for (AtomTypeData *atd1 = unweightedSQ.atomTypes().first(); atd1 != NULL; atd1 = atd1->next(), ++i)
			{
				...
				for (AtomTypeData *atd2 = atd1; atd2 != NULL; atd2 = atd2->next(), ++j)
				{
					...
					Interpolator::addInterpolated(combinedUnweightedSQ.at(globalI, globalJ), partialIJ,
								      factor);
		...
		if (augmentationStyle == RefineModule::PartialsAugmentation)
		{
			...
			for (AtomType *at1 = dissolve.atomTypes().first(); at1 != NULL; at1 = at1->next(), ++i)
			{
				...
				for (AtomType *at2 = at1; at2 != NULL; at2 = at2->next(), ++j)
				{
					...
					if (!scatteringMatrix_.addPartialReferenceData(data, at1, at2, factor,
										       (1.0 - augmentationParam)))
						return Messenger::error(
							"Refine: Failed to augment scattering matrix with partial %s-%s.\n",
							at1->name(), at2->name());
				}
			}
		}
		...
		Array2D<Data1D> &estimatedGR = GenericListHelper<Array2D<Data1D>>::realise(
			dissolve.processingModuleData(), "EstimatedGR", uniqueName_, GenericItem::InRestartFileFlag);
		...
		for (AtomType *at1 = dissolve.atomTypes().first(); at1 != NULL; at1 = at1->next(), ++i)
		{
			...
			for (AtomType *at2 = at1; at2 != NULL; at2 = at2->next(), ++j)
			{
				...
				expGR.setObjectTag(CharString("%s//EstimatedGR//%s//%s-%s", uniqueName_.get(), group->name(),
							      at1->name(), at2->name()));
		...
		PartialSet &summedUnweightedGR = GenericListHelper<PartialSet>::realise(
			dissolve.processingModuleData(), "UnweightedGR", uniqueName(), GenericItem::InRestartFileFlag);
		 ...
		 */
		Array2D<Data1D> &deltaSQ = GenericListHelper<Array2D<Data1D>>::realise(
			dissolve.processingModuleData(), "DeltaSQ", uniqueName_, GenericItem::InRestartFileFlag, &created);
		if (created)
		...
		for (AtomType *at1 = dissolve.atomTypes().first(); at1 != NULL; at1 = at1->next(), ++i)
		{
			...
			for (AtomType *at2 = at1; at2 != NULL; at2 = at2->next(), ++j)
			{
				...
				dSQ.setObjectTag(CharString("%s//DeltaSQ//%s//%s-%s", uniqueName_.get(), group->name(),
							    at1->name(), at2->name()));
		...
		Array2D<Data1D> &groupDeltaPhiR = GenericListHelper<Array2D<Data1D>>::realise(
		...
		Array2D<Data1D> &groupDeltaGR = GenericListHelper<Array2D<Data1D>>::realise(
			dissolve.processingModuleData(), CharString("DeltaGR_%s", group->name()), uniqueName_,
			GenericItem::InRestartFileFlag, &created);
		if (created)
		...
		Array2D<double> &minimumRadii = GenericListHelper<Array2D<double>>::realise(
			dissolve.processingModuleData(), "MinimumRadii", uniqueName_, GenericItem::InRestartFileFlag, &created);
		if (created)
		...
		for (AtomType *at1 = dissolve.atomTypes().first(); at1 != NULL; at1 = at1->next(), ++i)
		{
			...
			for (AtomType *at2 = at1; at2 != NULL; at2 = at2->next(), ++j)
			{
				...
				dPhiR.clear();
				dPhiR.setObjectTag(CharString("%s//DeltaPhiR//%s//%s-%s", uniqueName_.get(), group->name(),
							      at1->name(), at2->name()));

				// Grab delta g(r) container and make sure its object name is set
				Data1D &inversion = groupDeltaGR.at(i, j);
				inversion.setObjectTag(CharString("%s//Inversion//%s//%s-%s", uniqueName_.get(), group->name(),
								  at1->name(), at2->name()));
				...
				// deltaGRBond.setObjectTag(CharString("%s//DeltaGRBond//%s-%s", uniqueName_.get(), at1->name(),
				// at2->name()));
				//
				...
				if (inversionMethod == RefineModule::DirectFourierPotentialInversion)
				{
					...
					Fourier::sineFT(inversion, 1.0 / (2 * PI * PI * combinedRho.at(i, j)), ppDelta, ppDelta,
							ppRange, windowFunction);
				...
				else if (inversionMethod == RefineModule::DirectGaussianPotentialInversion)
				{
					...
					Messenger::print("Fitted function has error of %f%% with original delta S(Q) "
							 "(nGaussians = %i).\n",
							 error, gaussFit.nGaussians());

					// Store fitted parameters
					GenericListHelper<Array<double>>::realise(
						dissolve.processingModuleData(),
						CharString("%s-%s-GaussianX", at1->name(), at2->name()), uniqueName_,
						GenericItem::InRestartFileFlag) = gaussFit.x();
					GenericListHelper<Array<double>>::realise(
						dissolve.processingModuleData(),
						CharString("%s-%s-GaussianA", at1->name(), at2->name()), uniqueName_,
						GenericItem::InRestartFileFlag) = gaussFit.A();
					GenericListHelper<Array<double>>::realise(
						dissolve.processingModuleData(),
						CharString("%s-%s-GaussianFWHM", at1->name(), at2->name()), uniqueName_,
						GenericItem::InRestartFileFlag) = gaussFit.fwhm();

					// DEBUG
					// 					if (false)
					// 					{
					// 						deltaSQ.at(i,
					// j).save(CharString("%s-%s.orig", at1->name(), at2->name()));
					...
					// 						gaussFit.approximation(FunctionSpace::RealSpace, 1.0
					// / (2 * PI * PI * combinedRho.at(i,j)), ppDelta, ppDelta,
					// ppRange).save(CharString("%s-%s.ft", at1->name(), at2->name()));
					// 					}

					// Fourier transform the approximation, and store this as our inversion
					inversion = gaussFit.approximation(FunctionSpace::RealSpace,
									   1.0 / (2 * PI * PI * combinedRho.at(i, j)), ppDelta,
									   ppDelta, ppRange);
					dPhiR = inversion;
					dPhiR.values() *= -1.0;
				}
				else if (inversionMethod == RefineModule::PercusYevickPotentialInversion)
				{
					...
					cr = calculateCR(deltaSQ.at(i, j), 1.0 / (2.0 * PI * PI * combinedRho.at(i, j)),
							 ppDelta, ppDelta, ppRange, windowFunction);
				...
				else if (inversionMethod == RefineModule::HypernettedChainPotentialInversion)
				{
					...
					*/
					...
					return Messenger::error("Potential inversion method %i not accounted for.\n",
								inversionMethod);
				...
				for (int n = 0; n < dPhiR.nValues(); ++n)
				{
					...
					if (r < truncationStart)
						...
						y[n] *= 0.5 -
							0.5 * cos(PI * 0.5 * (r - truncationStart) / (truncationWidth * 0.5));
				}
	...
	for (AtomType *at1 = dissolve.atomTypes().first(); at1 != NULL; at1 = at1->next(), ++i)
	{
		...
		for (AtomType *at2 = at1; at2 != NULL; at2 = at2->next(), ++j)
		{
			Data1D &partialErrors = GenericListHelper<Data1D>::realise(
	...
	if (modifyPotential)
	{
		...
		while (ModuleGroup *group = groupIterator.iterate())
		{
			...
			if (!dissolve.processingModuleData().contains(CharString("DeltaPhiR_%s", group->name()), uniqueName()))
				return Messenger::error("Could not locate delta phi(r) data for group '%s'.\n", group->name());
			const Array2D<Data1D> &groupDeltaPhiR = GenericListHelper<Array2D<Data1D>>::value(
				dissolve.processingModuleData(), CharString("DeltaPhiR_%s", group->name()), uniqueName_,
				Array2D<Data1D>());
			...
			for (AtomType *at1 = dissolve.atomTypes().first(); at1 != NULL; at1 = at1->next(), ++i)
			{
				...
				for (AtomType *at2 = at1; at2 != NULL; at2 = at2->next(), ++j)
				{
					...
					if (onlyWhenErrorStable)
					{
						const Data1D &partialErrors = GenericListHelper<Data1D>::value(
							dissolve.processingModuleData(),
						...
						if (partialErrors.nValues() >= errorStabilityWindow)
						{
							double yMean;
							double grad =
								Regression::linear(partialErrors, errorStabilityWindow, yMean);
							double thresholdValue = fabs(0.001 * yMean);
							if (fabs(grad) < thresholdValue)
							{
								Messenger::print(
									"Error gradient of last %i points for partial %s-%s is "
									"%e %%/step (absolute threshold value is %e, stable = "
									"%s) so no "
									"potential adjustment will be made.\n",
									errorStabilityWindow, at1->name(), at2->name(), grad,
									thresholdValue,
									DissolveSys::btoa(fabs(grad) <= thresholdValue));
								continue;
							}
