...
bool RefineModule::process(Dissolve &dissolve, ProcessPool &procPool)
{
	...
	if (augmentationStyle == RefineModule::NoAugmentation)
		...
		Messenger::print("Refine: Augmentation of scattering matrix will be performed (style = '%s', parameter = %e).\n", matrixAugmentationStyles().keyword(augmentationStyle),
				 augmentationParam);
	if (autoMinimumRadii)
		Messenger::print("Refine: Minimum radii of generated potentials will be determined automatically (min/max = %f/%f Angstroms).\n", globalMinimumRadius, globalMaximumRadius);
	else
	...
	if (modifyPotential)
		...
		Messenger::print("Refine: Perturbations to interatomic potentials will be generated only (current potentials will not be modified).\n");
	if (onlyWhenEnergyStable)
		Messenger::print("Refine: Potential refinement will only be performed if all related Configuration energies are stable.\n");
	if (onlyWhenErrorStable)
		Messenger::print("Refine: Potential refinement will only be performed if all percentage errors with reference data are stable.\n");
	if (phiMax >= 0)
	...
	if (windowFunction.function() == WindowFunction::NoWindow)
		...
		Messenger::print("Refine: Window function to be applied in Fourier transforms of S(Q) to g(r) is %s (%s).", WindowFunction::functionType(windowFunction.function()),
	...
	while (ModuleGroup *group = groupIterator.iterate())
	{
		...
		for (AtomType *at1 = dissolve.atomTypes().first(); at1 != NULL; at1 = at1->next(), ++i)
		{
			...
			for (AtomType *at2 = at1; at2 != NULL; at2 = at2->next(), ++j)
				combinedUnweightedSQ.at(i, j).setObjectTag(CharString("%s//UnweightedSQ//%s//%s-%s", uniqueName(), group->name(), at1->name(), at2->name()));
		}
		...
		for (Module *module : targetModules)
		{
			...
			double rho = GenericListHelper<double>::value(dissolve.processingModuleData(), "EffectiveRho", module->uniqueName(), 0.0, &found);
			if (!found)
		...
		if (scatteringMatrix_.underDetermined())
			return Messenger::error("Can't proceed with an underdetermined scattering matrix. Select an augmentation style other than 'None'.\n");
		...
		Array2D<Data1D> &groupDeltaPhiR =
		    GenericListHelper<Array2D<Data1D>>::realise(dissolve.processingModuleData(), CharString("DeltaPhiR_%s", group->name()), uniqueName_, GenericItem::InRestartFileFlag, &created);
		if (created)
			groupDeltaPhiR.initialise(nTypes, nTypes, true);
		Array2D<Data1D> &groupDeltaGR =
		    GenericListHelper<Array2D<Data1D>>::realise(dissolve.processingModuleData(), CharString("DeltaGR_%s", group->name()), uniqueName_, GenericItem::InRestartFileFlag, &created);
		if (created)
			groupDeltaGR.initialise(nTypes, nTypes, true);
		// 	if (modifyBonds)
		// 	{
		// 		Array2D<Data1D>& deltaGRBond = GenericListHelper< Array2D<Data1D> >::realise(dissolve.processingModuleData(), "DeltaGRBond", uniqueName_,
		// GenericItem::InRestartFileFlag, &created); 		if (created) deltaGRBond.initialise(nTypes, nTypes, true);
		// 	}
		...
		for (AtomType *at1 = dissolve.atomTypes().first(); at1 != NULL; at1 = at1->next(), ++i)
		{
			...
			for (AtomType *at2 = at1; at2 != NULL; at2 = at2->next(), ++j)
			{
				...
				// 				Array2D<Data1D>& deltaGRBonds = GenericListHelper< Array2D<Data1D> >::retrieve(dissolve.processingModuleData(), "DeltaGRBond",
				...
				else if (inversionMethod == RefineModule::DirectGaussianPotentialInversion)
				{
					...
					GenericListHelper<Array<double>>::realise(dissolve.processingModuleData(), CharString("%s-%s-GaussianX", at1->name(), at2->name()), uniqueName_,
										  GenericItem::InRestartFileFlag) = gaussFit.x();
					GenericListHelper<Array<double>>::realise(dissolve.processingModuleData(), CharString("%s-%s-GaussianA", at1->name(), at2->name()), uniqueName_,
										  GenericItem::InRestartFileFlag) = gaussFit.A();
					GenericListHelper<Array<double>>::realise(dissolve.processingModuleData(), CharString("%s-%s-GaussianFWHM", at1->name(), at2->name()), uniqueName_,
					...
					// 						gaussFit.approximation(FunctionSpace::RealSpace, 1.0 / (2 * PI * PI * combinedRho.at(i,j)), ppDelta, ppDelta,
	...
	for (AtomType *at1 = dissolve.atomTypes().first(); at1 != NULL; at1 = at1->next(), ++i)
	{
		...
		for (AtomType *at2 = at1; at2 != NULL; at2 = at2->next(), ++j)
		{
			Data1D &partialErrors = GenericListHelper<Data1D>::realise(dissolve.processingModuleData(), CharString("PartialError_%s-%s", at1->name(), at2->name()), uniqueName_,
	...
	if (modifyPotential)
	{
		...
		while (ModuleGroup *group = groupIterator.iterate())
		{
			...
			const Array2D<Data1D> &groupDeltaPhiR =
			    GenericListHelper<Array2D<Data1D>>::value(dissolve.processingModuleData(), CharString("DeltaPhiR_%s", group->name()), uniqueName_, Array2D<Data1D>());
			...
			for (AtomType *at1 = dissolve.atomTypes().first(); at1 != NULL; at1 = at1->next(), ++i)
			{
				...
				for (AtomType *at2 = at1; at2 != NULL; at2 = at2->next(), ++j)
				{
					...
					if (onlyWhenErrorStable)
					{
						const Data1D &partialErrors =
						    GenericListHelper<Data1D>::value(dissolve.processingModuleData(), CharString("PartialError_%s-%s", at1->name(), at2->name()), uniqueName_);
						if (partialErrors.nValues() >= errorStabilityWindow)
						{
							...
							if (fabs(grad) < thresholdValue)
							{
								Messenger::print("Error gradient of last %i points for partial %s-%s is %e %%/step (absolute threshold value is %e, stable = %s) so no "
