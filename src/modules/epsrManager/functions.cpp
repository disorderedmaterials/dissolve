...
bool EPSRManagerModule::generateEmpiricalPotentials(std::string_view modulename, GenericList &moduleData, Dissolve &dissolve,
                                                    double averagedRho, std::optional<int> ncoeffp, double rminpt,
                                                    double rmaxpt, double sigma1, double sigma2)
{
    // Get coefficients array
    const auto &coefficients = moduleData.valueIf<Array2D<std::vector<double>>>("PotentialCoefficients", modulename);
