	...
	if (referenceFQ_.hasValidFileAndFormat())
	{
		...
		if (normType != NeutronSQModule::NoNormalisation)
		{
			...
			if (normType == NeutronSQModule::AverageOfSquaresNormalisation)
			{
				...
				Messenger::print(
					"NeutronSQ: Removed <b>**2 normalisation from reference data ('%s'), factor = %f.\n",
		...
		Data1D &storedData = GenericListHelper<Data1D>::realise(dissolve.processingModuleData(), "ReferenceData",
...
bool NeutronSQModule::process(Dissolve &dissolve, ProcessPool &procPool)
{
	...
	Messenger::print(
		"NeutronSQ: Calculating S(Q)/F(Q) over %f < Q < %f Angstroms**-1 using step size of %f Angstroms**-1.\n", qMin,
		qMax, qDelta);
	...
	if (qBroadening.function() == BroadeningFunction::NoFunction)
		...
		Messenger::print("NeutronSQ: Broadening to be applied in calculated S(Q) is %s (%s).",
				 BroadeningFunction::functionType(qBroadening.function()),
				 qBroadening.parameterSummary().get());
	if (saveUnweighted)
	...
	for (Configuration *cfg : targetConfigurations_)
	{
		...
		const bool sqUpToDate =
			DissolveSys::sameString(unweightedsq.fingerprint(),
						CharString("%i/%i", cfg->moduleData().version("UnweightedGR"),
		...
		if (includeBragg)
		{
			...
			for (int i = 0; i < unweightedsq.nAtomTypes(); ++i)
			{
				for (int j = i; j < unweightedsq.nAtomTypes(); ++j)
				{
					...
					braggPartials.at(i, j) /=
						cfg->usedAtomTypeData(i)->fraction() * cfg->usedAtomTypeData(j)->fraction();
				}
			}
		...
		PartialSet &weightedsq = GenericListHelper<PartialSet>::realise(cfg->moduleData(), "WeightedSQ", NULL,
										GenericItem::InRestartFileFlag, &created);
		if (created)
			weightedsq.setUpPartials(unweightedsq.atomTypes(), CharString("%s-%s", cfg->niceName(), uniqueName()),
						 "weighted", "sq", "Q, 1/Angstroms");
	...
	Data1D &repGR = GenericListHelper<Data1D>::realise(dissolve.processingModuleData(), "RepresentativeTotalGR",
							   uniqueName_, GenericItem::InRestartFileFlag);
