...
bool NeutronSQModule::setUp(Dissolve &dissolve, ProcessPool &procPool)
{
	...
	if (referenceFQ_.hasValidFileAndFormat())
	{
		...
		if (referenceData.constXAxis().lastValue() < qMax)
			Messenger::warn("Qmax limit of %e Angstroms**-1 for calculated NeutronSQ (%s) is beyond limit of reference data (Qmax = %e Angstroms**-1).\n", qMax, uniqueName(),
					referenceData.constXAxis().lastValue());
		else
		...
		if (referenceWindowFunction.function() == WindowFunction::NoWindow)
			...
			Messenger::print("Window function to be applied in Fourier transform of reference data is %s (%s).", WindowFunction::functionType(referenceWindowFunction.function()),
		...
		if (nTargetConfigurations() == 0)
			Messenger::warn("No configurations associated to module, so Fourier transform of reference data will use assumed atomic density of 0.1.\n");
...
bool NeutronSQModule::process(Dissolve &dissolve, ProcessPool &procPool)
{
	...
	if (windowFunction.function() == WindowFunction::NoWindow)
		...
		Messenger::print("NeutronSQ: Window function to be applied in Fourier transforms is %s (%s).", WindowFunction::functionType(windowFunction.function()),
	...
	if (referenceWindowFunction.function() == WindowFunction::NoWindow)
		...
		Messenger::print("Window function to be applied when calculating representative g(r) from S(Q) is %s (%s).", WindowFunction::functionType(referenceWindowFunction.function()),
				 referenceWindowFunction.parameterSummary().get());
	...
	if (qBroadening.function() == BroadeningFunction::NoFunction)
		...
		Messenger::print("NeutronSQ: Broadening to be applied in calculated S(Q) is %s (%s).", BroadeningFunction::functionType(qBroadening.function()), qBroadening.parameterSummary().get());
	if (saveUnweighted)
	...
	if (includeBragg)
	{
		...
		if (braggQBroadening.function() == BroadeningFunction::NoFunction)
			...
			Messenger::print("NeutronSQ: Additional broadening to be applied in calculated Bragg S(Q) is %s (%s).", BroadeningFunction::functionType(braggQBroadening.function()),
	...
	for (Configuration *cfg : targetConfigurations_)
	{
		...
		if (created)
			unweightedsq.setUpPartials(unweightedgr.atomTypes(), CharString("%s-%s", cfg->niceName(), uniqueName()), "unweighted", "sq", "Q, 1/Angstroms");
		...
		const bool sqUpToDate = DissolveSys::sameString(unweightedsq.fingerprint(),
								CharString("%i/%i", cfg->moduleData().version("UnweightedGR"), includeBragg ? cfg->moduleData().version("BraggReflections") : -1));
		...
		if (!SQModule::calculateUnweightedSQ(procPool, cfg, unweightedgr, unweightedsq, qMin, qDelta, qMax, cfg->atomicDensity(), windowFunction, qBroadening))
			return false;
		...
		if (includeBragg)
		{
			...
			if (!cfg->moduleData().contains("BraggReflections"))
				return Messenger::error("Bragg scattering requested to be included, but Configuration '%s' contains no reflection data.\n", cfg->name());
		...
		if (isotopologues_.contains(cfg))
		{
			...
			while (Isotopologues *topes = topeIterator.iterate())
			{
				...
				while (IsotopologueWeight *isoWeight = weightIterator.iterate())
					weights.addIsotopologue(spInfo->species(), spInfo->population(), isoWeight->isotopologue(), isoWeight->weight());
			}
		}
		...
		if (created)
			weightedsq.setUpPartials(unweightedsq.atomTypes(), CharString("%s-%s", cfg->niceName(), uniqueName()), "weighted", "sq", "Q, 1/Angstroms");
