...
// Print
void CharString::info() const { std::printf("CharString len = %i, end = %i : '%s'\n", size_, endPosition_, data_); }
...
// Get
const char *CharString::get() const { return (data_ != NULL ? data_ : ""); }

// Get length
int CharString::length() const { return (endPosition_ < size_ ? endPosition_ : size_); }
...
// Create empty array
void CharString::createEmpty(CharString &s) { createEmpty(s.size_); }

// Fill current string with specified character
void CharString::fill(char c)
{
    for (int n = 0; n < size_ - 1; ++n)
        data_[n] = c;
    data_[size_ - 1] = '\0';
}

// Returns the length of the current string
bool CharString::isEmpty() const { return (endPosition_ <= 0 ? true : false); }

// Return last character of string (before '\0')
char CharString::lastChar() const { return (endPosition_ == 0 ? '\0' : data_[endPosition_ - 1]); }
...
void CharString::eraseEnd(int n)
{
    if ((n - 1) >= endPosition_)
        n = endPosition_;
    if (n > 0)
        erase(endPosition_ - n, endPosition_ - 1);
}
...
{
    for (int n = 0; n < endPosition_; ++n)
        if (std::strchr(fromChars, data_[n]))
            data_[n] = toChar;
}
...
bool CharString::operator==(const CharString &s) const
{
    if (data_ == NULL)
    {
        if ((s.data_ == NULL) || (s.data_[0] == '\0'))
            return true;
        else
            return false;
    }
    else if (s.data_ == NULL)
        return (data_[0] == '\0');
...
void CharString::operator+=(const char *s) { strcat(s); }

// Conversion operators
CharString::operator const char *() const { return get(); }
...
// Return as double
double CharString::asDouble() const { return (data_ != NULL ? atof(data_) : 0.0); }

// Return as integer
int CharString::asInteger() const { return (data_ != NULL ? atoi(data_) : 0); }

// Returns contents as long integer
long int CharString::asLongInteger() const { return (data_ != NULL ? atol(data_) : 0); }
...
int CharString::rFind(char search, char stopat1, char stopat2) const
{
    ...
    {
        if (data_[result] == stopat1)
            return -1;
        if (data_[result] == stopat2)
            return -1;
        if (data_[result] == search)
            break;
    }
    return result;
}
...
char *CharString::strchr(char c) const { return std::strchr(data_, c); }
