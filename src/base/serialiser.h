...
template <typename T>
concept serialisablePointer = requires(T a, std::string name, SerialisedValue target) { a->serialise(name, target); };
...
template <typename... Contexts> class Serialisable
{
    ...
    virtual void serialise(std::string name, SerialisedValue &target) const = 0;
    ...
    {
        fromVector(vector, name, node, [](const auto &item) { return item->into_toml(); });
    }
    // A helper function to add the elements of a vector to a node under a name
    template <typename T>
