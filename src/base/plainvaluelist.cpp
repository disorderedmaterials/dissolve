...
// Add/set value
void PlainValueList::add(const char* name, PlainValue newValue, const char* description, int genericItemFlags)
{
...
{
	for (PlainValue* oldValue = values_.first(); oldValue != NULL; oldValue = oldValue->next) if (DUQSys::sameString(name, oldValue->name())) return oldValue;
