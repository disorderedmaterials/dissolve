...
{
    if (sameString(s, "true") || sameString(s, "on"))
        return true;
    return false;
}

// Convert boolean to string representation ("True" or "False")
const char *DissolveSys::btoa(bool b) { return (b ? "True" : "False"); }

// Convert boolean to string representation ("On" or "Off")
const char *DissolveSys::onOff(bool b) { return (b ? "On" : "Off"); }
...
bool DissolveSys::sameWildString(const char *wild, const char *s2, bool caseSensitive)
{
    ...
    if (*wild == '*')
    {
        ...
        if ((*(wild + 1) != '\0') && (*s2 == '\0'))
            ...
            return sameWildString(wild + 1, s2, caseSensitive) || sameWildString(wild, s2 + 1, caseSensitive);
    }
    else
    {
        // Wildcard '?' matches any single character, so increase both character positions and continue
        if (*wild == '?')
            return sameWildString(wild + 1, s2 + 1, caseSensitive);
    }

    // Not a wildcard, so do absolute character matching
    if (caseSensitive)
    {
        if (tolower(*wild) == tolower(*s2))
            return sameWildString(wild + 1, s2 + 1, caseSensitive);
    }
    else if (*wild == *s2)
        return sameWildString(wild + 1, s2 + 1, caseSensitive);
...
const char *DissolveSys::beforeLastChar(const char *s, char delim)
{
    ...
    for (count = 0; count < lastPos; ++count)
        result += s[count];
    return result;
}
...
const char *DissolveSys::afterStr(const char *s, const char *search)
{
    ...
    if (c == NULL)
        return "";
    for (const char *d = &search[0]; *d != '\0'; ++d)
        c++;
    return c;
}
